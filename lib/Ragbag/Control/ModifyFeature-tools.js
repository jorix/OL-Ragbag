...
OpenLayers.Control.ModifyFeature = OpenLayers.Class(OpenLayers.Control, {
    ...
    dragVertex: function(vertex, pos, pixel) {
        ...
        } else if (vertex._tool === 'point' || isRealPoint) {
            // dragging a point
            mustMoveTools = true;
        ...
        if (mustMoveTools) {
            this.moveTools(this.toolbar, x, y);
            if (vertex._tool === 'drag') {
                this.moveTools(this.dragTools, x, y);
            }
        } else {
     ...
     * Method: moveTools
     */
    moveTools: function(toolArray, x, y) {
    ...
    collectTools: function() {
        ...
        var center = bounds.getCenterLonLat(),
            ...
            _toolbar = [],
            // no scope
            addTool = function(toolArray, toolName, x, y, toolMove) {
                var toolGeometry = new OpenLayers.Geometry.Point(x, y);
                toolGeometry.move = toolMove;
                toolArray.push(_self.createVertex(toolName, toolGeometry));
            },
        ...
        if (!isLikeAPoint) {
            if (mode & MODES.DRAG) {
                addTool(_dragTools, "drag", center.lon, center.lat,
                    function(x, y) {
                        _geometry.move(x, y);
                    }
                );
