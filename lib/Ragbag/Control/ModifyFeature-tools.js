...
OpenLayers.Control.ModifyFeature = OpenLayers.Class(OpenLayers.Control, {
     ...
     *     see <activateLayer>.  Default is false.
     */
    ...
    initialize: function(layer, options) {
        ...
        var dragCallbacks = {
            down: function(pixel) {
                ...
                var feature = this.getFeatureFromEvent(
                        this.activeLayer, this.layers, this.handlers.drag.evt);
                if (feature) {
            ...
            up: function(pixel) {
                ...
                if (vertex &&
                    vertex === this.activeLayer.getFeatureFromEvent(
                                                             handlerDrag.evt)) {
    ...
    /**
     * APIMethod: activateLayer
     * Activate the layer to modify features, layer is moved to the top.
     */
    activateLayer: function(layer) {
        if (this.active && this.activeLayer !== layer) {
            this.deactivateLayer();
    ...
    /**
     * APIMethod: deactivateLayer
     * Deactivate the active layer, if exist the active layer is moved back to
     *     the position determined by the map's layers.
     */
     ...
     *     of the feature is automatically activated (see <activateLayer>)
     * Use this method when the control is created without specifying the layer
     *     argument and without <layers> option or in <standalone> mode, see
     *     <constructor>.
     *
    ...
    unselectFeature: function(keepActiveLayer) {
        ...
        if (feature) {
            ...
            this.modified = false;
            !keepActiveLayer && this.activateLayer(this.layer);
        }
    },
    ...
    collectVertices: function(mode) {
        ...
        var layer = feature.layer;
        layer.addFeatures(_virtualVertices, {silent: true});
        layer.addFeatures(_vertices, {silent: true});
    ...
    moveLayerBack: function() {
        var layer = this.activeLayer;
        if (layer) {
            var index = layer.getZIndex() - 1;
            if (index >= this.map.Z_INDEX_BASE['Feature']) {
                layer.setZIndex(index);
            } else {
                this.map.setLayerZIndex(layer,
                    this.map.getLayerIndex(layer));
            }
        }
    },
     ...
     *
     * Parameters:
     * layer - {<OpenLayers.Feature.Vector>} The active layer.
     * layers - Array({<OpenLayers.Feature.Vector>}) Layers over which control
     *     acts to modify features.
     ...
     */
    getFeatureFromEvent: function(layer, layers, evt) {
        var feature = null;
        if (layer) {
        ...
        if (!feature && layers.length){
            ...
            var layerDiv = layer && layer.div.style.display !== "none" ?
                                                               layer.div : null;
            if (layerDiv) {
