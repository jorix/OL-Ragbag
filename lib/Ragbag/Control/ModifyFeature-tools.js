...
OpenLayers.Control.ModifyFeature = OpenLayers.Class(OpenLayers.Control, {
    ...
    dragStart: function(feature, pixel) {
        ...
        if (!feature._sketch && isPoint && this.virtualVertices.length) {
            // if exists then drag the virtual point instead of the real point
    ...
    pressVertex: function(vertex, pixel) {
        ...
        if (vertex._tool === 'delete') {
            ...
            feature.layer.drawFeature(feature, this.standalone ? undefined :
                                        'select');
            this.dragComplete();
        }
    },
    ...
    dragVertex: function(vertex, pos, pixel) {
        ...
        } else if (toolName === 'point' || isRealPoint) {
            // dragging a point
            mustMoveTools = true;
        ...
        layer.drawFeature(feature, this.standalone ? undefined :
                                        'select');
    ...
    removeTools: function() {
        if (this.dragTools.length > 0) {
            this.layer.destroyFeatures(this.dragTools, {silent: true});
            this.dragTools = [];
        }
        if (this.toolbar.length > 0) {
            this.layer.destroyFeatures(this.toolbar, {silent: true});
            this.toolbar = [];
        }
    },
    ...
    collectVertices: function(mode) {
        ...
        var _pushPoint = function(geometry) {
            if (_usePoint) {
                ...
                _vertices.push(control.createVertex('point', false, geometry));
            }
        };
    ...
    collectTools: function() {
        ...
        if (!isLikeAPoint) {
            if (mode & MODES.DRAG) {
                addTool(_dragTools, "drag", false, center.lon, center.lat,
                    function(x, y) {
                        _geometry.move(x, y);
                    }
                );
